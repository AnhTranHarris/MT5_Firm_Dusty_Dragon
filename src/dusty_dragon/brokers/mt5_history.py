...
class MT5HistoryTransport(Protocol):
    """Read-only MT5 history boundary for uncertain execution outcomes."""
...
class MT5HistoryAdapter:
    """Normalize broker order/deal history without granting write authority."""
...
def _normalize_order_status(state: int) -> BrokerOrderHistoryStatus:
    # MT5 ORDER_STATE_FILLED=4, CANCELED=2, REJECTED=3. Other states stay
    # UNKNOWN so Dusty never manufactures certainty from an unfamiliar state.
    if state == 4:
...
def _timestamp_utc(raw: Mapping[str, Any], key: str) -> datetime:
    return datetime.fromtimestamp(_required_int(raw, key), tz=UTC)
