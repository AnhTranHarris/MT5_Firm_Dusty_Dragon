...
def opened_reconciliation(connection, *, broker_order_id: str | None):
    ...
    opened_at = datetime(2026, 8, 26, 14, 0, 5, tzinfo=UTC)
...
def history():
    return MT5HistoryAdapter(FakeHistoryTransport()).read_execution_history(
...
def test_mt5_history_adapter_normalizes_orders_and_deals() -> None:
    ...
    assert observed.orders[0].broker_order_id == "42"
    assert observed.orders[0].status is BrokerOrderHistoryStatus.FILLED
    assert observed.orders[1].status is BrokerOrderHistoryStatus.CANCELED
    assert observed.deals[0].broker_order_id == "42"
...
def test_matching_deal_confirms_execution() -> None:
    ...
    result = service.resolve_from_history(
...
def test_transport_error_without_broker_order_id_is_not_guessed() -> None:
    ...
    observed = BrokerExecutionHistory(
        orders=(
            BrokerOrderHistoryRecord(
                broker_order_id="99",
