(() => {
  ...
  /*
   * PERFORMANCE TIMEFRAME v3.3 — adaptive fiscal reporting control.
   * ------------------------------------------------------------------
   * UX CONTRACT
   * - SPATIAL FULL / SPATIAL REDUCED: use a four-stop range slider.
   * - Moving between stops morphs the fiscal graph through deterministic numeric
   *   interpolation (dates, target, realized-series shape and chart scale).
   * - NO MOTION / HIGH CONTRAST: the slider disappears and four ordinary buttons
   *   appear BELOW the graph/timeline. Selection is immediate and animation-free.
   * - Actual bars NEVER render to the right of the authoritative as-of date.
   * - The bright-blue TODAY line remains fixed at the reporting cut while the
   *   financial target continues through the selected fiscal horizon.
   *
   * DUSTY MANAGEMENT REPORTING POLICY (UI Lab)
   * - U.S. federal fiscal convention: Oct 1 -> Sep 30.
   * - Fiscal year is named by END year (FY2026 = Oct 1 2025-Sep 30 2026).
   *
   * WINDOWS / PRODUCTION HANDOFF
   * - Dusty Core owns immutable {period_start, period_end, as_of, actual[], target[]}.
   * - Native UI may animate only between two already-authoritative read models.
   *   Interpolation is PRESENTATION geometry, never accounting truth.
   * - If Windows animation/reduced-motion policy disables motion, use the static
   *   button path and render the destination read model directly.
   * - Never synthesize future actuals, move as_of, or mutate MT5/broker/risk/ledger.
   * - Cancel an in-flight presentation morph if display policy changes to no-motion.
   */
  ...
  function shapedValues(finalValue, count) {
    ...
    return Array.from({length:Math.max(1,count)}, (_, index) => {
      const shapeIndex = Math.round(index * (shape.length - 1) / Math.max(1, count - 1));
      return finalValue * shape[shapeIndex];
    });
  }
  ...
  function monthConfig() {
    ...
    const actual = values.map((value, index) => ({
      date: new Date(start.getTime() + (today.getTime() - start.getTime()) * index / Math.max(1, count - 1)),
      value
    }));
  ...
  function quarterConfig() {
    ...
    const actual = values.map((value, index) => ({
      date: new Date(start.getTime() + (today.getTime() - start.getTime()) * index / Math.max(1, weeks - 1)),
      value
    }));
  ...
  function fiveYearConfig() {
    ...
    return {id:"fiveYear",label:"5 YEAR",short:"5Y",start,end,target:MONTHLY_TARGET*60,actual,note:`${fiscalYearLabel(start)}-FY${fiscalYearEnd(new Date(start.getFullYear()+4,start.getMonth(),1)).getFullYear()} PLAN`};
  }
  ...
  function sampleSeries(series, normalized) {
    ...
    return lerp(Number(series[left].value || 0), Number(series[right].value || 0), fraction);
  }
  ...
  function blendedConfig(from, to, progress) {
    ...
    const actual = Array.from({length:count}, (_, index) => {
      ...
      return {
        date:new Date(lerp(start.getTime(), actualEnd.getTime(), normalized)),
        value:lerp(sampleSeries(from.actual,normalized), sampleSeries(to.actual,normalized), t)
      };
    });
    return {
      id:t < .5 ? from.id : to.id,
      label:t < .5 ? from.label : to.label,
      short:t < .5 ? from.short : to.short,
      start,
      end,
      target:lerp(from.target,to.target,t),
      actual,
      note:t < .5 ? from.note : to.note,
      transitioning:true
    };
  }
  ...
  function chartSvg(config, expanded) {
    ...
    const bars = actual.map(point=>`<rect class="investor-actual-bar" x="${x(point.date)-barWidth/2}" y="${y(point.value)}" width="${barWidth}" height="${Math.max(1,height-pad.b-y(point.value))}" rx="2"><title>${fmtDate(point.date)} · realized ${pct(point.value)}</title></rect>`).join("");
    const goalPoints = Array.from({length:65},(_,index)=>{const fraction=index/64;const date=new Date(config.start.getTime()+span*fraction);return `${x(date)},${y(target*fraction)}`;}).join(" ");
